...
if inferFormat
    ...
    if ~feof(fid)
        ...
        format = updateformat(fid, format, delimiter, whiteSpace, treatAsEmpty);
...
function format = updateformat(fid, format, delimiter, whiteSpace, treatAsEmpty)
...
while(~feof(fid))
    ...
    for jj=1:numel(rawline)
        ...
        % doesn't change)
        if nnz(rawline{jj}==delimiter)+1 ~= nvars
